...
A prebuilt local package can also be created with `npm run build` then `npm pack`. Copy that `.tgz` file to GrokBot's computer and run `npm install -g /path/to/grokbot-openai-bridge-0.1.0.tgz`, then `bridge init`. The package contains the compiled CLI, skill and documentation; source tests are run from the source checkout.
...
| `repo_url` | Exact Git origin URL without embedded credentials |
| `repo_path` | Absolute path on GrokBot's computer; Windows JSON needs escaped backslashes or `/` |
| `working_branch` | `bridge/daily-line-agent-1`, or your task branch |
...
The clone parent directory must already exist. Git uses the host's normal credential helper and stops if access is unavailable. `repo prepare` refuses dirty trees; manually preserve existing work first. It creates/selects the isolated branch. An existing record cannot be overwritten through `project add`; changing pinned identity requires a reviewed database migration in V0.
...
3. Run `bridge start daily-line --task "Continue DL-Agent-1."`.
...
6. Inspect `bridge history daily-line` and the received instruction. If the status is `READY_FOR_NEXT_CODEX_TURN`, run `bridge continue daily-line`. The bridge sends that exact instruction to Codex.
...
8. If the run is `WAITING_FOR_CHATGPT`, perform the new outbox handoff to the **same** ChatGPT thread. It contains the code/test/diff evidence. Import the review response.
...
The export index maps each raw artifact to its run, turn, kind and timestamp.
